...
namespace CMVSTools
{
	namespace CMV
	{
		...
		{
			m_ifsCMV = OpenFileBinaryStream(m_wsCMV);
			m_ifsCMV.read((char*)&m_Header, sizeof(m_Header));
			m_wsFolder = PathRemoveExtension(m_wsCMV) + L"\\";
			ReadIndex();
		}
		...
		void CMVEditor::ReadIndex()
		{
			...
			m_ifsCMV.read((char*)index_ptr, index_size);
		...
		void CMVEditor::SaveFrame(CMV_Entry& ceEntry)
		{
			...
			m_ifsCMV.seekg(offset, std::ios::beg);
			m_ifsCMV.read(m_AutoMem[size], size);
			SaveFileViaPath((m_wsFolder + name).c_str(), m_AutoMem, size);
		}
