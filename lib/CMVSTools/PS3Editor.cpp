...
namespace CMVSTools
{
	namespace PS3Editor
	{
		...
		PS3Extract::PS3Extract(std::wstring& wsPath, uint32_t dwCodePage, bool isFullText) :
		...
		bool PS3Extract::CreateDumpFile()
		{
		...
		void PS3Extract::DumpText()
		{
			...
			for (auto& pStr : m_vecppStr)
			{
				...
				else
				{
					...
					Rut::StrX::StrToWStr(msText, wsText, m_dwCodePage);
		...
		PS3Inset::PS3Inset(std::wstring& wsPath, uint32_t dwCodePage) :
		...
		bool PS3Inset::InsetTextFile()
		{
			...
			if (!err && fpTextFile != NULL)
			{
				while (feof(fpTextFile) == 0)
				{
					...
					wsText = newText;
					Rut::StrX::WStrToStr(wsText, msText, m_dwCodePage);
