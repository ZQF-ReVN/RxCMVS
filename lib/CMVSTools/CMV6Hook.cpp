...
#include "../../third/libwebp/include/encode.h"
#include "../../third/libwebp/include/decode.h"
#pragma comment(lib,"../../third/libwebp/lib/libwebp.lib")
...
namespace CMVSTools
{
	namespace CMV6Hook
	{
		...
		void CMV6FrameDump()
		{
			Rut::ConsoleX::SetConsole(L"CMV6FrameDump");

			InitBMPInfo(&g_bFile, &g_bInfo);

			Rut::HookX::DetourAttachFunc(&orgDecJBPDBuffer, newDecJBPD);
			Rut::HookX::DetourAttachFunc(&orgGetJBPXBuffer, newGetJBPDBuffer);
		}
		...
		void CMV6DecodeWebP()
		{
			...
			Rut::HookX::DetourAttachFunc(&orgDecJBPDBuffer, DecWebP);
		}

		//Set Console Input .CMV File Path Unpack All Resources And Decode JBPX
		void UnPackCMVThread()
		{
			Rut::ConsoleX::SetConsole(L"UnPackCMV / Decode JBPX Thread");
